...
function QuestionForm() {
  ...
  const handleDeleteConfirmed = () => {
    ...
    axios
      .delete(`https://docback.hasthiya.org/questions/deleteQuestion`, {
        data: { questionId: [questionIdToDelete] },
      })
      .then((response) => {
        console.log("Deleted successfully:", response.data);

        // Update allQuestions state to remove the deleted question
        setAllQuestions((prevQuestions) =>
          prevQuestions.filter((question) => question.id !== questionIdToDelete)
        );

        // Update filteredQuestions state to remove the deleted question
        setFilteredQuestions((prevFilteredQuestions) =>
          prevFilteredQuestions.filter(
            (question) => question.id !== questionIdToDelete
          )
        );

        // Update orderedQuestions state to remove the deleted question
        setOrderedQuestions((prevOrderedQuestions) =>
          prevOrderedQuestions.filter(
            (question) => question.id !== questionIdToDelete
          )
        );
    ...
    <div>
      <div className="notification">
        <Snackbar
          ...
          anchorOrigin={{ vertical: "top", horizontal: "right" }} // Set anchorOrigin to top right corner
        >
          <MuiAlert
            elevation={6}
            variant="filled"
        ...
        <Grid item xs={12}>
          ...
          >
            ...
            <td colSpan="5" style={{ textAlign: "right" }}>
              <Button
                variant="contained"
                color={updateOrderMode ? "primary" : "primary"} // Color changes based on mode
